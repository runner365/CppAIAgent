...
inline std::string get_now_str() {
    ...
    // 获取当前本地时间（包含毫秒）
    SYSTEMTIME st;
    GetLocalTime(&st);  // Windows 特有函数，获取本地时间（精确到毫秒）

    char dscr_sz[80];
    // 格式化时间字符串（注意月份在 SYSTEMTIME 中是从 1 开始的，与 Linux 的 tm_mon 不同）
    sprintf_s(dscr_sz, sizeof(dscr_sz),  // Windows 推荐使用 sprintf_s 替代 sprintf（安全版本）
        "%04d-%02d-%02d %02d:%02d:%02d.%03d",
        st.wYear,         // 年份（直接是完整年份，无需 +1900）
        st.wMonth,        // 月份（1-12，无需 +1）
        st.wDay,          // 日期
        st.wHour,         // 小时
        st.wMinute,       // 分钟
        st.wSecond,       // 秒
        st.wMilliseconds  // 毫秒（0-999）
    );
